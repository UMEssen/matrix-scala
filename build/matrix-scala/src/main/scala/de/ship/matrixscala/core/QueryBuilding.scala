...
object QueryBuilding {
  ...
  implicit def optionQueryBuilder[T](implicit
      tBuilder: QueryComponentBuilder[T]
  ): QueryComponentBuilder[Option[T]] = {
  ...
  implicit def seqQueryBuilder[T](implicit
      tBuilder: QueryComponentBuilder[T]
  ): QueryComponentBuilder[Seq[T]] = {
    object OptionBuilder extends QueryComponentBuilder[Seq[T]] {
      override def build(data: Seq[T]): Option[QueryData] =
        Some(data.flatMap(tBuilder.build).mkString(","))
    }
    OptionBuilder
  }

  def buildQuery(entries: Option[(String, QueryData)]*): Map[String, QueryData] =
    entries.flatten.toMap

  def fromQueryEntry[T](t: (String, T))(implicit
      builder: QueryComponentBuilder[T]
  ): Option[(String, QueryData)] = builder.build(t._2).map(t._1 -> _)
